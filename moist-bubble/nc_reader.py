...
class nc_reader(nc_base_reader):
    ...
    def _get_derived_dataset(self, step, name, copy_periodic):
        if name == "vorticity_magnitude":
            x_vor = self.get_dataset(
                step=step, name="x_vorticity", copy_periodic=copy_periodic
            )
            y_vor = self.get_dataset(
                step=step, name="y_vorticity", copy_periodic=copy_periodic
            )
            z_vor = self.get_dataset(
                step=step, name="z_vorticity", copy_periodic=copy_periodic
            )
            return np.sqrt(x_vor**2 + y_vor**2 + z_vor**2)
        if name == "helicity":
            u = self.get_dataset(
                step=step, name="x_velocity", copy_periodic=copy_periodic
            )
            v = self.get_dataset(
                step=step, name="y_velocity", copy_periodic=copy_periodic
            )
            w = self.get_dataset(
                step=step, name="z_velocity", copy_periodic=copy_periodic
            )
            xi = self.get_dataset(
                step=step, name="x_vorticity", copy_periodic=copy_periodic
            )
            eta = self.get_dataset(
                step=step, name="y_vorticity", copy_periodic=copy_periodic
            )
            zeta = self.get_dataset(
                step=step, name="z_vorticity", copy_periodic=copy_periodic
            )
        ...
        if name == "cross_helicity_magnitude":
            ...
            uvec[:, :, :, 0] = u
            uvec[:, :, :, 1] = self.get_dataset(
                step=step, name="y_velocity", copy_periodic=copy_periodic
            )
            uvec[:, :, :, 2] = self.get_dataset(
                step=step, name="z_velocity", copy_periodic=copy_periodic
            )
            ovec[:, :, :, 0] = self.get_dataset(
                step=step, name="x_vorticity", copy_periodic=copy_periodic
            )
            ovec[:, :, :, 1] = self.get_dataset(
                step=step, name="y_vorticity", copy_periodic=copy_periodic
            )
            ovec[:, :, :, 2] = self.get_dataset(
                step=step, name="z_vorticity", copy_periodic=copy_periodic
            )
