...
def get_epic_parcel_shape(input_file_name, time_step=0):
    ...
    ds_nc.close()
    return bb1, bb2, bb2, bb3, bb4, bb5


def get_epic_parcel_buoyancy(input_file_name, time_step=0):
    ds_nc = nc.Dataset(input_file_name)
    b = ds_nc.variables["buoyancy"][time_step, :]
    ds_nc.close()
    return buoyancy
...
def get_epic_parcel_humidity(input_file_name, time_step=0):
    ds_nc = nc.Dataset(input_file_name)
    h = ds_nc.variables["humidity"][time_step, :]
    ds_nc.close()
    return h
...
def get_projection_coordinates(input_file_name, refinement_factor):
    ds_nc = nc.Dataset(input_file_name)
...
def produce_epic_cross_section(
    input_file_name, loc=0.0, cross_type="xz", kernel="third", refinement_factor=8
):
    ...
    bb1, bb2, bb2, bb3, bb4, bb5 = get_epic_parcel_shape(input_file_name)
    vol = get_epic_parcel_volume(input_file_name)
    len_condense, q_scale = get_idealised_moist_parameters(input_file_name)
    xp, yp, zp = get_projection_coordinates(input_file_name, refinement_factor)
    ...
    for i_thread in range(NTHREADS):
        ...
        for i in range(len(vol)):
            ...
            if cross_type == "xy" and (loc < z_now - r_max or loc > z_now + r_max):
                continue
            elif cross_type == "xz" and (loc < y_now - r_max or loc > y_now + r_max):
                continue
            elif cross_type == "yz" and (loc < x_now - r_max or loc > x_now + r_max):
                continue
            ...
            if cross_type == "xy":
                ...
                for ii in range(xlower, xupper + 1):
                    for jj in range(ylower, yupper + 1):
                        xdist = ii * dx_project + x_origin - x_now
                        ydist = jj * dy_project + y_origin - y_now
                        zdist = loc - z_now
...
def write_field_to_file(field, field_x, field_y, field_name, out_file, cross_type, mode):
